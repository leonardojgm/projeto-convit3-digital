...
export default function EventoCard(props: EventoCardProps) {
    return (
        <View style={[bgZinc900, {width: 500}]}>
            <Image source={{uri: props.evento.imagem}} style={[{ height: 150}, wFull, roundedMd]}/>
            <View style={[p4]}>
                <Text style={[textWhite, fontBold, textLg, textCenter]}>{props.evento.nome}</Text>
                <Text style={[textZinc400, textXs, textCenter, w9_10]}>{props.evento.descricao}</Text>
            </View>
        </View>
    );
}