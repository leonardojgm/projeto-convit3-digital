...
export default function CampoEntrada(props: CampoEntradaProps) {
    ...
    return (
        <div className={`flex flex-col gap-2 ${props.outterClassName ?? ""}`}>
            ...
            <input {...propsInput()} type={props.type ?? "text"} className="w-full px-3 py-2 border border-white/20 focus:border-white/50 rounded-md bg-black/50" />
            {props.erro && <span className="pl-2 text-sm text-red-500">{props.erro}</span>}
            {!props.erro && props.observacao && <span className="pl-2 text-xs text-yellow-300">{props.observacao}</span>}
        </div>
    );
}